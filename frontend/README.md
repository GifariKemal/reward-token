...
| `chainId` disebut eksplisit di tiap panggilan tulis  | Dibuktikan dengan connector tiruan, bukan diasumsikan: `writeContract` mengambil chain dari `connection.chainId` milik wallet dan viem MELEWATI `assertCurrentChain`, jadi tanpa `chainId` transaksinya sungguh terkirim ke jaringan yang sedang aktif di wallet. Yang paling berbahaya `approve`: allowance dibaca di chain 97, izinnya ditandatangani di chain lain                  |
...
| `chainId` juga di pembacaan dan watcher event        | Jaring pengaman, BUKAN penambal bug, dan ini koreksi atas alasan yang sempat saya tulis. Jalur baca memakai `config.state.chainId`, dan wagmi menolak menyinkronkannya ke chain yang tidak terdaftar di `config.chains`, jadi selama daftar itu cuma berisi bscTestnet nilainya selalu 97. Berguna begitu `chains` bertambah                                                           |
