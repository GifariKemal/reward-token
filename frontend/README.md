...
| Masukan | Penjaga |
|---|---|
| `rulesURI` dan `proofURI` dari pengguna | Ditulis apa adanya ke kontrak, dan itu memang desainnya. Yang mengambil isinya adalah backend, dan di sanalah penjaga SSRF berada (`uriAman()` di `services/judge.ts`) |
...
| Chain yang aktif di wallet | `chainId` eksplisit di setiap tulis, wagmi menolak sebelum tanda tangan |
